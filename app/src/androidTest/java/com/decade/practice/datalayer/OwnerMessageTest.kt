...
class OwnerMessageTest {
      ...
      fun sendTextToThird() {
            ...
            runTest {
                  withContext(Dispatchers.Main) {
                        ...
                        Assert.assertEquals(session.dialogRepository.list()[0].conversation.partner.username, "third")
      ...
      fun sendSecondTextToThird() {
            ...
            runTest {
                  withContext(Dispatchers.Main) {
                        val session = accountRepository.currentSession!!
                        Assert.assertEquals(session.dialogRepository.list()[0].conversation.partner.username, "second")
                        session.messageService.send(conversation, "ekkk")
                  }
            }
            Thread.sleep(1500)
