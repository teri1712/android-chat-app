...
class MessageCountTest {
    ...
    fun testCountThirdMessages() = runTest {
        withContext(Dispatchers.Main) {
            ...
            val conversation = testConversation("first", "third")
            Assert.assertEquals(2, session.persistentContext.get(conversation).messages.size)
    ...
    fun testCountSecondMessagesAfterSend() {
        ...
        runTest {
            withContext(Dispatchers.Main) {
                ...
                Assert.assertEquals("second", list[0].partner.username)
                val conversation = testConversation("first", "second")
                Assert.assertEquals(2, session.persistentContext.get(conversation).messages.size)
                Assert.assertNotNull(session.persistentContext.get(conversation).messages[0].textEvent)
                Assert.assertNotNull("hello nami", session.persistentContext.get(conversation).messages[0].textEvent?.content)
...
class SeenMessageTest {
    ...
    fun secondSendSeen() {
        ...
        runTest {
            ...
            val persistentRepo = session.persistentContext
